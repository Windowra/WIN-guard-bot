...
@bot.tree.command(guild=GUILD, description="Ban user")
async def ban(i:discord.Interaction, member:discord.Member, reason:str="No reason"):
    await member.ban(reason=reason); await i.response.send_message(f"Banned {member}", ephemeral=True)

@bot.tree.command(guild=GUILD, description="Kick user")
async def kick(i:discord.Interaction, member:discord.Member, reason:str="No reason"):
    await member.kick(reason=reason); await i.response.send_message(f"Kicked {member}", ephemeral=True)

@bot.tree.command(guild=GUILD, description="Timeout user")
async def timeout(i:discord.Interaction, member:discord.Member, minutes:int, reason:str="No reason"):
    await member.timeout(timedelta(minutes=minutes), reason=reason); await i.response.send_message(f"Timed {minutes}m", ephemeral=True)

@bot.tree.command(guild=GUILD, description="Untimeout")
...
@bot.tree.command(guild=GUILD, description="Slowmode")
...
@bot.tree.command(guild=GUILD, description="Warn")
...
@bot.tree.command(guild=GUILD, description="Warnings")
...
@bot.tree.command(guild=GUILD, description="Clear warns")
...
@bot.tree.command(guild=GUILD, description="Nick")
...
async def serverinfo(i:discord.Interaction):
    await i.response.send_message(embed=discord.Embed(title=i.guild.name))

@bot.tree.command(guild=GUILD, description="User info")
async def userinfo(i:discord.Interaction, member:discord.Member):
    await i.response.send_message(embed=discord.Embed(title=member.name).set_thumbnail(url=member.display_avatar.url))
...
@bot.tree.command(guild=GUILD, description="Say")
